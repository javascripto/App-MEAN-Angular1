  ...
  angular.module('primeiraApp').component('paginator', {
    ...
    controller: [
      ...
      function($location) {
        this.$onInit = function() {
          ...
          this.pagesArray = Array(pages).fill(0).map((v, k) => k + 1 )
    ...
    <ul ng-if="$ctrl.needPagination" class="pagination pagination-sm no-margin pull-right">
      <li ng-if="$ctrl.hasPrev">
        <a href="{{ $ctrl.url }}?page={{ $ctrl.current - 1 }}"><i class="fa fa-arrow-left"></i></a>
      </li>
      ...
      <li ng-if="$ctrl.hasNext">
        <a href="{{ $ctrl.url }}?page={{ $ctrl.current + 1 }}"><i class="fa fa-arrow-right"></i></a>
      </li>
    </ul>`
  })
})()